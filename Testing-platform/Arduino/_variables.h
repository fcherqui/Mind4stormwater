...
float UPPER_LIM = 1.900;          //Used to switch direction of pump and stop program (max=1.9, more than that and the floor is wet)
...
const int NB_DS_PER_LEVEL=    1;    //Number of Dallas temperature measures per water level (default=100)
const int NB_DHT_PER_LEVEL=   100;    //Number of DHT temperature/air humidity measures per water level (default=100)

#define TIME_PUMP      7000    //time (in milliseconds) for the pump to work (at each step), default=7000, , i.e. 7 seconds (5 seconds = 0.03 m --> 6mm/second), min = 5s because of the computer memory
#define STABILIZE_TIME 15000   //Time to stabilize water level after pump off (defaut=15000, i.e. 15 seconds)
const float Offset PROGMEM =    0.143;  //Offset (distance OTT sensor to the 0, default=0.150, if you have to change this, it meas that the reference sensor has drifted...)
